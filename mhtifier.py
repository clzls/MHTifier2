...
def main():
  ...
  if args.unpack:
    ...
    a = email.message_from_file(mht, policy=email.policy.SMTPUTF8)

    parts = a.get_payload() # Multiple parts, usually?
    if not type(parts) is list:
      parts = [a] # Single part, convert to list.
    ...
    for p in parts: # walk() for a tree, but I'm guessing MHT is never nested?
      ...
      if p['Content-Transfer-Encoding'] == '7bit':
        p.replace_header('Content-Transfer-Encoding','8bit')
      
      content = p.get_content()
      if isinstance(content, str):
        content = content.encode('utf-8') # Re-encode strings (html, hopefully) into UTF-8 bytes.
